...
-- Hack to create user if it does not exist because you cannot `IF EXISTS` users in 5.6
GRANT USAGE ON *.* TO 'zotero_master' IDENTIFIED BY 'password';
DROP USER 'zotero_master';
CREATE USER 'zotero_master' IDENTIFIED BY 'password';
GRANT SELECT, INSERT, UPDATE, DELETE ON zotero_master.* TO 'zotero_master';
